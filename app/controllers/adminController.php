    ...
    class adminController extends mainModel{
        ...
        public function registrarAdminControlador(){
            ...
            if($registrar_administrativo->rowCount()==1){
			 	$alerta=[
			 		"tipo"=>"limpiar",
			 		"titulo"=>"Administrativo registrado",
			 		"texto"=>"El administrativo ".$nombre." ".$apellido." se registro con exito",
			 		"icono"=>"success"
			 	];
			}else{
			 	...
			 	$alerta=[
			 		"tipo"=>"simple",
			 		"titulo"=>"Ocurrió un error inesperado",
			 		"texto"=>"No se pudo registrar el administrativo, por favor intente nuevamente",
					"icono"=>"error"
			 	];
			}
		...
		public function eliminarAdminControlador(){
            ...
            public function actualizarAdminControlador(){
                ...
                # Verificando usuario #
                $datos=$this->ejecutarConsulta("SELECT a.id idAdministrativos, p.id idPersonas, u.id idUsuarios, c.id idContactos, dir.id idDirecciones, d.id idDocumentaciones,
													a.sector Sector, concat(p.nombre,' ',p.apellido) nombreApellido, a.fecha_creacion FechaCreacion, pro.nombre Provincia,
                ...
                if($datos->rowCount()<=0){
                    ...
                    $id_administrativo = $datos['idAdministrativos'];
                }
    
                //ALmacenando los datos que vienen de la vista
                //Persona
                $nombre = $this->limpiarCadena($_POST['adminNombre']);
                $apellido = $this->limpiarCadena($_POST['adminApellido']);
                $documentacionTipo = $this->limpiarCadena($_POST['adminDocumentacionTipo']);
                $documentacionNumero = $this->limpiarCadena($_POST['adminDocumentacionNumero']);
                $cuil = $this->limpiarCadena($_POST['adminCuil']);
                $numeroSegSocial = $this->limpiarCadena($_POST['adminNumeroSegSocial']);
                //Usuario
                $usuario = $this->limpiarCadena($_POST['adminUsuario']);
                $clave1 = $this->limpiarCadena($_POST['adminClave1']);
                $clave2 = $this->limpiarCadena($_POST['adminClave2']);
                //Contacto
                $correo = $this->limpiarCadena($_POST['adminCorreo']);
                $telefono = $this->limpiarCadena($_POST['adminTelefono']);
                //Direccion
                $calle = $this->limpiarCadena($_POST['adminCalle']);
                $barrio = $this->limpiarCadena($_POST['adminBarrio']);
                $provincia = $this->limpiarCadena($_POST['adminProvincia']);
                //Administrativo
                $sector = $this->limpiarCadena($_POST['adminSector']);
                ...
                if($this->verificarDatos("[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ ]{4,20}",$sector)){
                    //Definimos todo los valores de un array para la alerta
                    $alerta = [
                        "tipo" => "simple",
                        "titulo" => "Ocurrió un error inesperado",
                ...
                try{
                    ...
                    $condicion_administrativo=[
                        "condicion_campo"=>"id",
                        "condicion_marcador"=>":ID",
                        "condicion_valor"=>$id_administrativo
                    ];
                    $ac_administrativo=$this->actualizarDatos("administrativos",$administrativos_datos_up,$condicion_administrativo);
                    ...
                    $alerta=[
                        "tipo"=>"recargar",
                        "titulo"=>"Administrativo actualizado",
            ...
            public function listarAdminAvanzadoControlador($pagina,$registros,$url,$busqueda){
                ...
                if($aKeyword=='' && $provinciaFil=='' && $ordenarPorFil==''){
                    ...
                    //exit();
                    $query = "SELECT a.id idAdministrativos, p.id idPersonas, u.id idUsuarios, c.id idContactos, dir.id idDirecciones, d.id idDocumentaciones,
                                a.sector Sector, concat(p.nombre,' ',p.apellido) nombreApellido, a.fecha_creacion FechaCreacion, pro.nombre Provincia
							  FROM administrativos a
							  JOIN personas p
							  ON p.id = a.id_personas
                    ...
                    if($aKeyword!=''){
                        ...
                        $query.=" AND (p.nombre LIKE '%".$aKeyword[0]."%' OR p.apellido LIKE '%".$aKeyword[0]."%' OR a.sector LIKE '%".$aKeyword[0]."%') ";

                        $consulta_total.=" AND (p.nombre LIKE '%".$aKeyword[0]."%' OR p.apellido LIKE '%".$aKeyword[0]."%' OR a.sector LIKE '%".$aKeyword[0]."%') ";

                        //recorremos la busqueda textual por si hay mas de una palabra
                        for($i=1; $i< count($aKeyword); $i++){
                            if(!empty($aKeyword[$i])){
                                $query.=" AND (p.nombre LIKE '%".$aKeyword[$i]."%' OR p.apellido LIKE '%".$aKeyword[$i]."%' OR a.sector LIKE '%".$aKeyword[$i]."%') ";

                                $consulta_total.=" AND (p.nombre LIKE '%".$aKeyword[$i]."%' OR p.apellido LIKE '%".$aKeyword[$i]."%' OR a.sector LIKE '%".$aKeyword[$i]."%') ";
                            }
                        }
                    }
                ...
                if($ordenarPorFil == 2){
                    ...
                    $consulta_total.=" ORDER BY a.fecha_creacion DESC ";
                }

                if($ordenarPorFil == 3){
                    $query.=" ORDER BY a.fecha_creacion ASC ";
                    $consulta_total.=" ORDER BY a.fecha_creacion ASC ";
                }
				if($ordenarPorFil == 4){
                ...
                $datos = $datos->fetchAll();
                $_SESSION['tablaAdministrativos'] = $datos;
                $total = $this->ejecutarConsulta($consulta_total);
                $total = (int) $total->fetchColumn();
                    ...
                    foreach($datos as $rows){
                        $tabla.='
                                <tr>
                                    ...
                                    <td>'.date("d-m-Y",strtotime($rows['FechaCreacion'])).'</td>
                                    <td>
                                        <a href="'.APP_URL.'adminPhoto/'.$rows['idUsuarios'].'/" class="btn btn-secondary">
                                            <i class="fa-solid fa-image"></i>
                                        </a>
                                    </td>
                                    <td>
                                        <a href="'.APP_URL.'adminUpdate/'.$rows['idUsuarios'].'/" class="btn btn-success">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                    </td>
                                    <td>
