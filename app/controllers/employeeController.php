...
class employeeController extends mainModel{
            ...
            public function registrarEmpleadoControlador(){
                ...
                if($registrar_medico->rowCount()==1){
                     $alerta=[
                         "tipo"=>"limpiar",
            ...
            public function listarEmpleadoAvanzadoControlador($pagina, $registros, $url, $busqueda) {
                ...
                $tipoEmpleado = $busqueda[0];
                $ordenarPorFil = $busqueda[1];
                $aKeyword = explode(" ", $busqueda[2]);
                // echo $busqueda[2];
                // exit();
                // Base query
                $query = "SELECT em.id idEmpleado, p.id idPersona, te.id idTipoEmpleado, p.cuil Cuil,
                            concat(p.nombre,' ',p.apellido) nombreApellido, te.descripcion tipoEmpleado, em.fecha_creacion fechaAlta, pro.nombre provincia
                          FROM empleados em
                          JOIN personas p 
                          ON p.id = em.id_personas
                          JOIN tipo_empleados te
                          ON te.id = em.id_tipo_empleados
                          JOIN direcciones d
                          ON d.id = p.id_direcciones
                          JOIN provincias pro
                          ON pro.id = d.id_provincias
                          WHERE em.flag_mostrar = 1";
            
                $consulta_total = "SELECT count(em.id) 
                                    FROM empleados em
                                    JOIN personas p 
                                    ON p.id = em.id_personas
                                    JOIN tipo_empleados te
                ...
                if($tipoEmpleado!=''){
                    $query.=" AND te.id = '$tipoEmpleado' ";
                    $consulta_total.=" AND te.id = '$tipoEmpleado' ";
                }
                    ...
                    foreach ($datos as $rows) {
                        $tabla .= '
                            <tr>
                                ...
                                <td>' . $rows['tipoEmpleado'] . '</td>
                                <td>' . $rows['provincia'] . '</td>
                                <td>' . date("d-m-Y", strtotime($rows['fechaAlta'])) . '</td>
                                <td>
                                    <a href="' . APP_URL . 'userPhoto/' . $rows['idPersona'] . '/" class="btn btn-secondary">
                                        <i class="fa-solid fa-image"></i>
                                    </a>
                                </td>
                                <td>
                                    <a href="' . APP_URL . 'userUpdate/' . $rows['idPersona'] . '/" class="btn btn-success">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>
                                </td>
                                <td>
                                    <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/employeeAjax.php" method="post" autocomplete="off">
                                        <input type="hidden" name="modulo_empleado" value="eliminar">
                                        <input type="hidden" name="persona_id" value="' . $rows['idPersona'] . '">
            ...
            public function eliminarEmpleadoControlador(){
                ...
                //Verificar
                $datos = $this->ejecutarConsulta("SELECT m.id idMedico, p.id idPersona, m.numero_colegiado numColegiado,
                ...
                if($eliminar_medico->rowCount()==1 && $eliminar_persona->rowCount()==1 &&
                    ...
                    $alerta=[
                        ...
                        "texto"=>"El medico ".$datos['Nombre']." ".$datos['Apellido']." se eliminó con exito",
                        "icono"=>"success"
                    ];
            ...
            public function actualizarEmpleadoControlador(){
                ...
                # Verificando usuario #
                $datos=$this->ejecutarConsulta("SELECT m.id idMedico, u.id idUsuario, c.id idContacto, dir.id idDireccion, p.id idPersona, pro.id idProvincia,
                                                ...
                                                u.nombre_usuario usuario, aes_decrypt(u.clave, 'PEPE') clave,
                                                m.fecha_creacion fechaCreacion, m.ultima_modificacion ultimaModificacion, m.numero_colegiado numeroColegiado, m.clave_seguridad_social claveSegSocial
                                                sr.id idSituacionRevista, sr.descripcion situacionRevista, e.id idEspecialidades, e.descripcion especialidades 
                                                c.telefono telefono, c.correo correo, dir.calle calle, dir.barrio barrio
                                                FROM medicos m
                                                JOIN especialidades e
                                                ON e.id = m.id_especialidades
                                                JOIN situacion_revista sr
                ...
                try{
                    ...
                    $ac_medico=$this->actualizarDatos("administrativos",$medicos_datos_up,$condicion_medico);
                    ...
                    $alerta=[
                        ...
                        "texto"=>"Los datos del medico ".$datos['Nombre']." ".$datos['Apellido']." se actualizaron correctamente",
                        "icono"=>"success"
                    ];
