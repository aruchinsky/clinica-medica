<?php
    ...
    class tecnicController extends mainModel{
        ...
        public function registrarTecnicoControlador(){
                ...
                //Persona
                $nombre = $this->limpiarCadena($_POST['tecnicoNombre']);
                $apellido = $this->limpiarCadena($_POST['tecnicoApellido']);
                $documentacionTipo = $this->limpiarCadena($_POST['tecnicoDocumentacionTipo']);
                $documentacionNumero = $this->limpiarCadena($_POST['tecnicoDocumentacionNumero']);
                $cuil = $this->limpiarCadena($_POST['tecnicoCuil']);
                $numeroSegSocial = $this->limpiarCadena($_POST['tecnicoNumeroSegSocial']);
                //Usuario
                $usuario = $this->limpiarCadena($_POST['tecnicoUsuario']);
                $clave1 = $this->limpiarCadena($_POST['tecnicoClave1']);
                $clave2 = $this->limpiarCadena($_POST['tecnicoClave2']);
                //Contacto
                $correo = $this->limpiarCadena($_POST['tecnicoCorreo']);
                $telefono = $this->limpiarCadena($_POST['tecnicoTelefono']);
                //Direccion
                $calle = $this->limpiarCadena($_POST['tecnicoCalle']);
                $barrio = $this->limpiarCadena($_POST['tecnicoBarrio']);
                $provincia = $this->limpiarCadena($_POST['tecnicoProvincia']);
                //Tecnico de Salud
                $ocupacion = $this->limpiarCadena($_POST['tecnicoOcupacion']);
			...
			if($this->verificarDatos("[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{4,40}",$ocupacion)){
                //Definimos todo los valores de un array para la alerta
                $alerta = [
                    "tipo" => "simple",
                    "titulo" => "Ocurrió un error inesperado",
            ...
            if($_FILES['tecnicoFoto']['name']!="" && $_FILES['tecnicoFoto']['size']>0){
                ...
                if(mime_content_type($_FILES['tecnicoFoto']['tmp_name'])!="image/jpeg" &&
                    mime_content_type($_FILES['tecnicoFoto']['tmp_name'])!="image/png"){
                ...
                if($_FILES['tecnicoFoto']['size']/1024>5012){
                    $alerta=[
                        "tipo"=>"simple",
                        "titulo"=>"Ocurrió un error inesperado",
                ...
                switch(mime_content_type($_FILES['tecnicoFoto']['tmp_name'])){
                    case "image/jpeg":
                        $foto=$foto.".jpg";
                        break;
                    case "image/png":
                ...
                if(!move_uploaded_file($_FILES['tecnicoFoto']['tmp_name'],$img_dir.$foto)){
                    $alerta=[
                        "tipo"=>"simple",
                        "titulo"=>"Ocurrió un error inesperado",
            ...
            try{
                    ...
                    // 6-1. Cargamos el array de Empleados
                    $empleado_datos_reg=[
                        [
                            "campo_nombre"=>"id_personas",
                            "campo_marcador"=>":idPersonas",
                    ...
                    // 6-2. Registramos los datos de Empleados
                    $registrar_empleado=$this->guardarDatos("empleados",$empleado_datos_reg);

				// 7-1. Cargamos el array de tecnico de salud
				$tecnico_datos_reg=[
					[
						"campo_nombre"=>"ocupacion",
						"campo_marcador"=>":Ocupacion",
						"campo_valor"=>$ocupacion
					],
					[
						"campo_nombre"=>"id_personas",
						"campo_marcador"=>":idPersonas",
				...
				// 7-2. Registramos los datos de tecnico de salud
				$registrar_tecnico=$this->guardarDatos("tecnico_salud",$tecnico_datos_reg);
            ...
            if($registrar_tecnico->rowCount()==1){
			 	$alerta=[
			 		"tipo"=>"limpiar",
			 		"titulo"=>"Técnico de Salud registrado",
			 		"texto"=>"El técnico ".$nombre." ".$apellido." se registro con exito",
			 		"icono"=>"success"
			 	];
			}else{
			 	...
			 	$alerta=[
			 		"tipo"=>"simple",
			 		"titulo"=>"Ocurrió un error inesperado",
			 		"texto"=>"No se pudo registrar el técnico, por favor intente nuevamente",
					"icono"=>"error"
			 	];
			}
		...
		public function listarTecnicoControlador($pagina,$registros,$url,$busqueda){
		...
		public function eliminarTecnicoControlador(){
			...
			$datos = $this->ejecutarConsulta("SELECT ts.id idTecnicoSalud, e.id idEmpleado, p.id idPersona, c.id idContacto, dir.id idDireccion, d.id idDocumentacion,
                                                     ts.ocupacion Ocupacion, p.nombre Nombre, p.apellido Apellido, u.foto Foto
			...
			if($datos->rowCount()<=0){
                ...
                $id_tecnico_salud = $datos['idTecnicoSalud'];
            ...
            //Eliminamos el tecnico
            $eliminar_tecnico= $this->eliminarRegistro("tecnico_salud","id",$id_tecnico_salud);

			if($eliminar_tecnico->rowCount()==1){
				...
				$alerta=[
					"tipo"=>"recargar",
					"titulo"=>"Técnico eliminado",
					"texto"=>"El tecnico ".$datos['Nombre']." ".$datos['Apellido']." se eliminó con exito",
					"icono"=>"success"
				];
            ...
            public function actualizarTecnicoControlador(){
    
                $id=$this->limpiarCadena($_POST['tecnico_id']);
    
                # Verificando usuario #
                $datos=$this->ejecutarConsulta("SELECT ts.id idTecnicoSalud, p.id idPersonas, u.id idUsuarios, c.id idContactos, dir.id idDirecciones, d.id idDocumentaciones,
													ts.ocupacion Ocupacion, concat(p.nombre,' ',p.apellido) nombreApellido, ts.fecha_creacion FechaCreacion, pro.nombre Provincia,
                                                    aes_decrypt(u.clave,'PEPE') Clave
												FROM tecnico_salud ts
												JOIN personas p
												ON p.id = ts.id_personas
												JOIN contactos c
                ...
                if($this->verificarDatos("[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ ]{4,20}",$ocupacion)){
                    //Definimos todo los valores de un array para la alerta
                    $alerta = [
                        "tipo" => "simple",
                        "titulo" => "Ocurrió un error inesperado",
                ...
                try{
                    ...
                    // 5-1. Cargamos el array de Administrativos
                    $tecnico_datos_up=[
                        [
                            "campo_nombre"=>"ocupacion",
                            "campo_marcador"=>":Ocupacion",
                    ...
                    $condicion_tecnico=[
                        "condicion_campo"=>"id",
                        "condicion_marcador"=>":ID",
                        "condicion_valor"=>$id_tecnico_salud
                    ];
                    $ac_tecnico_salud=$this->actualizarDatos("tecnico_salud",$tecnico_datos_up,$condicion_tecnico);
                    ...
                    $alerta=[
                        "tipo"=>"recargar",
                        "titulo"=>"Técnico de Salud actualizado",
            ...
            public function listarTecnicoAvanzadoControlador($pagina,$registros,$url,$busqueda){
                ...
                if($aKeyword=='' && $provinciaFil=='' && $ordenarPorFil==''){
                    //echo "Prueba ".$provinciaFil;
                    //exit();
                    $query = "SELECT ts.id idTecnicoSalud, p.id idPersonas, u.id idUsuarios, c.id idContactos, dir.id idDirecciones, d.id idDocumentaciones,
                                ts.ocupacion Ocupacion, concat(p.nombre,' ',p.apellido) nombreApellido, ts.fecha_creacion FechaCreacion, pro.nombre Provincia
							  FROM tecnico_salud ts
							  JOIN personas p
							  ON p.id = ts.id_personas
                              ...
                              JOIN usuarios u
                              ON p.id = u.id_personas
                              WHERE ts.flag_mostrar = 1";

                    $consulta_total = "SELECT count(ts.id) 
                                        FROM tecnico_salud ts
                                        JOIN personas p
                                        ON p.id = ts.id_personas
                                        JOIN contactos c
                    ...
                    if($aKeyword!=''){
                        ...
                        $query.=" AND (p.nombre LIKE '%".$aKeyword[0]."%' OR p.apellido LIKE '%".$aKeyword[0]."%' OR ts.ocupacion LIKE '%".$aKeyword[0]."%') ";

                        $consulta_total.=" AND (p.nombre LIKE '%".$aKeyword[0]."%' OR p.apellido LIKE '%".$aKeyword[0]."%' OR ts.ocupacion LIKE '%".$aKeyword[0]."%') ";

                        //recorremos la busqueda textual por si hay mas de una palabra
                        for($i=1; $i< count($aKeyword); $i++){
                            if(!empty($aKeyword[$i])){
                                $query.=" AND (p.nombre LIKE '%".$aKeyword[$i]."%' OR p.apellido LIKE '%".$aKeyword[$i]."%' OR ts.ocupacion LIKE '%".$aKeyword[$i]."%') ";

                                $consulta_total.=" AND (p.nombre LIKE '%".$aKeyword[$i]."%' OR p.apellido LIKE '%".$aKeyword[$i]."%' OR ts.ocupacion LIKE '%".$aKeyword[$i]."%') ";
                            }
                        }
                    }
                ...
                if($ordenarPorFil == 2){
                    $query.=" ORDER BY ts.fecha_creacion DESC ";
                    $consulta_total.=" ORDER BY ts.fecha_creacion DESC ";
                }

                if($ordenarPorFil == 3){
                    $query.=" ORDER BY ts.fecha_creacion ASC ";
                    $consulta_total.=" ORDER BY ts.fecha_creacion ASC ";
                }
				if($ordenarPorFil == 4){
                    ...
                    foreach($datos as $rows){
                        $tabla.='
                                <tr>
                                    ...
                                    <td>'.date("d-m-Y",strtotime($rows['FechaCreacion'])).'</td>
                                    <td>
                                        <a href="'.APP_URL.'tecnicPhoto/'.$rows['idUsuarios'].'/" class="btn btn-secondary">
                                            <i class="fa-solid fa-image"></i>
                                        </a>
                                    </td>
                                    <td>
                                        <a href="'.APP_URL.'tecnicUpdate/'.$rows['idUsuarios'].'/" class="btn btn-success">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                    </td>
                                    <td>
                                        <form class="FormularioAjax" action="'.APP_URL.'app/ajax/tecnicAjax.php" method="post" autocomplete="off">
                                            <input type="hidden" name="modulo_tecnico" value="eliminar">
